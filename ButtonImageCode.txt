...
// This version will read the root directory and any files it finds it will
...
void loop() {
  ...
  if (buttonState == HIGH){
    drawSdJpeg("/X.jpg", 0, 0);
    Serial.println("we made it");
    return;
  }
  buttonState2 = digitalRead(buttonPin2);
  if (buttonState2 == HIGH){
    drawSdJpeg("/II.jpg", 0, 0);
    Serial.println("we made it");
    return;
  }
...
void jpegRender(int xpos, int ypos) {
  ...
  uint32_t min_w = min(mcu_w, (uint16_t)(max_x % mcu_w));
  uint32_t min_h = min(mcu_h, (uint16_t) (max_y % mcu_h));
